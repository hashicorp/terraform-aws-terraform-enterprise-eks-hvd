...
resource "aws_security_group_rule" "rds_allow_ingress_from_nodegroup" {
  ...
  description              = "Allow TCP/5432 (PostgreSQL) inbound to RDS cluster from TFE EKS node group."

  security_group_id = aws_security_group.rds_allow_ingress.id
}

resource "aws_security_group_rule" "rds_allow_ingress_from_sg" {
  count = var.sg_allow_ingress_to_rds != null ? 1 : 0

  type                     = "ingress"
  from_port                = 5432
  to_port                  = 5432
  protocol                 = "tcp"
  source_security_group_id = var.sg_allow_ingress_to_rds
  description              = "Allow TCP/5432 (PostgreSQL) inbound to RDS cluster from specified security group ID."

  security_group_id = aws_security_group.rds_allow_ingress.id
}

resource "aws_security_group_rule" "rds_allow_ingress_from_cidr" {
  count = var.cidr_allow_ingress_to_rds != null ? 1 : 0

  type        = "ingress"
  from_port   = 5432
  to_port     = 5432
  protocol    = "tcp"
  cidr_blocks = var.cidr_allow_ingress_to_rds
  description = "Allow TCP/5432 (PostgreSQL) inbound to RDS cluster from specified CIDR ranges."
